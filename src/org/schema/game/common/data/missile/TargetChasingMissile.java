...
public abstract class TargetChasingMissile extends Missile {
    ...
    private float missileSpeedSetting = 1.5f; //Todo:Get base missile speed from server config

    //Todo: Make these configurable values in blockBehaviorConfig.xml;
    private float launchSpeed = 1000f;
    private float maneuverabilityMultiplier = 1.0f;
    private float maxAccelerationMultiplier = 1.5f;
    //
    ...
    public void onSpawn() {
        //Inserted code
        //maxAcceleration = something;
        //Todo:Come up with an algorithm to calculate max acceleration based on missile hp (more hp = less max acceleration)
        acceleration = 0.1f;
        setSpeed((getSpeed() * acceleration) + launchSpeed);
        //
    }
    ...
    public Vector3f getTargetRelativePosition(Vector3f out){
        ...
        if(getTarget() == null){
            ...
            random.setSeed(getTarget().getId() + getId());
            getTarget().getAimingAtRelativePos(out, getOwner(), getTarget(), random, 0);
        }
    ...
    public Transform getTargetPosition(Transform out){
        ...
        if(getTarget() == null){
            ...
            if (ownSector != null) {
                getTarget().calcWorldTransformRelative(ownSector.getId(), ownSector.pos);
...
//				System.err.println("MISSILE POS "+updateCount+" based on "+currentTar.targetPosition+"; : ON "+(isOnServer() ? "SERVER" : "CLIENT")+": "+out.origin+"; TARGET: "+getTarget()+"; "+(isOnServer() ? getTargetPosition(new Transform()).origin : "ddd"));
    ...
    /* (non-Javadoc)
     * @see org.schema.game.common.data.world.SimpleTransformableSendableObject#updateLocal(org.schema.schine.graphicsengine.core.Timer)
     */
    @Override
    ...
    protected void onDeadServer() {
        ...
        if(getTarget() != null){
            ((GameServerState) getState()).getController().getMissileController()
                    .getMissileManager().targetManager.unregisterOne(getTarget().getId());
        }
        ((GameServerState) getState()).getController().getMissileController()
    ...
    public void onClientDie(int hitId) {
        ...
        if(getTarget() != null){
            ((GameClientState)getState()).getController().getClientMissileManager().targetManager.unregisterOne(getTarget().getId());
        }
